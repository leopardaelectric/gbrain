...
export async function extractMentionsFromDb(
  ...
  // Keep the Vammo fork's bounded incremental path: a daily --since scan
  // queries only recently touched pages instead of enumerating every page ref.
  ...
  for (const { slug, source_id } of remaining) {
    ...
    // (#4304: the --since filter moved to the
